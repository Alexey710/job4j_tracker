...
public class EasyStreamTest {
    @Test
    public void whenSourceIsNumders() {
        var rsl = EasyStream.of(10, 20, 30).collect();
        assertThat(rsl, is(List.of(10, 20, 30)));
    }

    @Test
    public void whenDoubleFilterAndDoubleMap() {
        var rsl = EasyStream.of(1, 2, 3, 10, 20, 30)
                .filter(a -> a % 2 == 0)
                .filter(b -> b < 30)
                .map(c -> c / 2)
                .map(g -> g * 100)
                .collect();
        assertThat(rsl, is(List.of(100, 500, 1000)));
    }
