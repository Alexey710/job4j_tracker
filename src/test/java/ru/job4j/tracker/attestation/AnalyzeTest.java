package ru.job4j.tracker.attestation;
...
public class AnalyzeTest {
    ...
    @Test
    public void whenListOfSubjectAverage3Subject() {
        List<Tuple> average = Analyze.averageScoreByPupil(
                List.of(
                        new Pupil("Ivanov", List.of(new Subject("Math", 100), new Subject("Lang", 100))),
                        new Pupil("Petrov", List.of(new Subject("Math", 60), new Subject("Lang", 60))),
                        new Pupil("Sidorov", List.of(new Subject("Astronomy", 50), new Subject("Lang", 80)))
                ).stream()
        );
        assertThat(average, is(List.of(
                new Tuple("Lang", 80D),
                new Tuple("Math", 80D),
                new Tuple("Astronomy", 50D)
    ...
    public void whenBestSubject() {
        Tuple best = Analyze.bestSubject(
                List.of(
                        ...
                        new Pupil("Petrov", List.of(new Subject("Math", 60), new Subject("Lang", 60))),
                        new Pupil("Sidorov", List.of(new Subject("Math", 100), new Subject("Lang", 60)))
                ).stream()
        );
        assertThat(best, is(new Tuple("Math", 260D)));
    }
}